#include "sched.h"
#include "utils.h"
#include "memory.h"
#include "uart.h"
#include "fdt.h"
#include <stdint.h>
#include "trap.h"
#include "cpio.h"
#include "timer.h"
#include "plic.h"
#include "paging.h"
#include "vfs.h"
#include "tmpfs.h"
#include "ramfs.h"
#include "devfs.h"

uint64_t hart_id;

char input[128];
int input_idx = 0;

void shell_main_thread() {
    char c;
    int special_c = 0;
    
    uart_puts("opi-rv2> ");
    while (1) {
        // 核心邏輯：從核心的 Ring Buffer 拿字元
        // 如果 Buffer 沒資料，這個呼叫會觸發 schedule() 讓出 CPU
        c = uart_getc(); 

        if (c == '\r' || c == '\n') {
            uart_putc('\n');
            input[input_idx] = '\0';

            // 解析指令
            char* args[5] = {"", "", "", "", ""};
            int idx = 0;
            char* cmd = input;
            char* end = input;

            // 簡單的 strtok 模擬邏輯
            while (*end != '\0') {
                if (*end == ' ') {
                    *end = '\0';
                    end++;
                    while (*end == ' ') end++; // 跳過多餘空格
                    if (idx < 5) args[idx++] = end;
                } else {
                    end++;
                }
            }

            // 指令判斷
            if (strcmp(cmd, "exec") == 0) {
                struct task_struct* child = create_user_process(args[0]);
                if (child) {                    
                    // Kernel Shell 必須等待子程序結束, 這樣才不會跟子程序搶 UART 的字元
                    while (child->state != THREAD_ZOMBIE) {
                        schedule(); // 讓出 CPU 給子程序跑
                    }
                    
                } else {
                    uart_printf("[Kernel] Failed to execute %s\n", args[0]);
                }
            } 
            else if (strcmp(cmd, "help") == 0) {
                uart_puts("Available commands:\n");
                uart_puts("\thelp\t- show all commands.\n");
                uart_puts("\thello\t- print Hello World!\n");
                uart_puts("\texec\t- execute typed user-program.\n");
            } 
            else if (strcmp(cmd, "hello") == 0) {
                uart_puts("Hello World!\n");
            }
            else {
                if (strlen(cmd) != 0) uart_printf("%s: Command not found!\n", cmd);
            }

            // 重置狀態
            input_idx = 0;
            uart_puts("opi-rv2> ");

        } else if (c == 127 || c == '\b') {
            if (input_idx > 0) {
                uart_puts("\b \b");
                input_idx--;
            }
        } else if (c >= 32 && c <= 126) {
            // 處理 Escape Sequence (如方向鍵)
            if (special_c != 0) {
                special_c++;
                if (special_c == 3) special_c = 0;
                continue;
            }
            if (input_idx < 127) {
                uart_putc(c);
                input[input_idx++] = c;
            }
        } else if (c == 27) {
            special_c = 1;
        }
    }
}

int do_exec(struct task_struct *task, const char *filename) {
    uint64_t size = 0;
    void *code_start = cpio_get_file(filename, &size); 
    if (!code_start) return -1;

    extern void clear_user_space_mappings(unsigned long *pgd);
    clear_user_space_mappings(task->pgd);

    task->code_pa = virt_to_phys((unsigned long)code_start); 
    task->code_size = size;
    task->user_stack = 0;

    struct pt_regs *regs = (struct pt_regs *)(task->kernel_sp - sizeof(struct pt_regs));
    memzero(regs, sizeof(struct pt_regs));
    regs->tp = (unsigned long)task;
    
    // 填寫 user va
    regs->epc = 0x0;                      // U-mode 程式進入點
    regs->sp = 0x3fffffc000 + STACK_SIZE;   // U-mode 棧頂
    
    // 設定 sstatus：SPP=0 (降權到 U-mode)
    unsigned long sstatus;
    asm volatile("csrr %0, sstatus" : "=r"(sstatus));
    sstatus &= ~(1UL << 8); // 清除 SPP
    sstatus |= (1UL << 5);  // 開啟 SPIE
    regs->sstatus = sstatus;

    return 0;
}

extern char _start[];
extern char _end[];

void start_kernel(uint64_t hartid, uint64_t fdt){
    uart_printf("start: %x\n", _start);
    uart_printf("end: %x\n", _end);
    hart_id = hartid;
    fdt = phys_to_virt(fdt);
    uart_init();
    plic_init(hart_id, fdt);
    irq_enable();
    memory_init(fdt);
    
    cpio_init(fdt);
    cpio_base = (void*)phys_to_virt(cpio_base);
    
    enable_timer_interrupt();
    set_freq(fdt);
    init_timer();

    // 註冊 tmpfs, ramfs 到核心驅動陣列中
    if (register_filesystem(&tmpfs_driver) != 0) {
        uart_puts("Panic: Failed to register tmpfs!\n");
        while(1);
    }
    if (register_filesystem(&ramfs_driver) != 0) {
        uart_puts("Panic: Failed to register ramfs!\n");
        while(1);
    }
    if (register_filesystem(&devfs_driver) != 0) {
        uart_puts("Panic: Failed to register ramfs!\n");
        while(1);
    }

    // 配置 rootfs 的掛載點控制塊
    rootfs = (struct mount*)allocate(sizeof(struct mount));
    if (!rootfs) {
        uart_puts("Panic: Failed to allocate rootfs mount structure!\n");
        while(1);
    }

    // 呼叫 tmpfs 的設置函數來初始化根目錄 vnode 與內部節點
    if (tmpfs_driver.setup_mount(&tmpfs_driver, rootfs) != 0) {
        uart_puts("Panic: Failed to setup rootfs mount!\n");
        while(1);
    }

    rootfs->mountpoint = rootfs->root;
    active_mounts[active_mount_count++] = rootfs;

    uart_puts("\n\n----------\n");
    uart_puts("\nStarting kernel ...\n\n");

    asm volatile("move tp, %0" : : "r"(thread_create(idle)));

    // 建立 /ramfs 並在此路徑做 ramfs 的 mount
    vfs_mkdir("/ramfs");
    vfs_mount("/ramfs", "ramfs");
    // 建立 /ramfs 並在此路徑做 ramfs 的 mount
    vfs_mkdir("/dev");
    vfs_mount("/dev", "devfs");
    
    thread_create(shell_main_thread);
    sbi_set_timer(get_ticks() + cpu_freq/32);
    idle();
    
    while(1);
}