...
void startup_allocator(uint64_t start, uint64_t size, uint64_t fdt) {
    ...
    if (offset >= 0) {
        ...
        if (ptr) {
            if (len == 4) rg[idx].start = phys_to_virt(fdt32_to_cpu(ptr));
            else rg[idx].start = phys_to_virt(fdt64_to_cpu(ptr));
        }
        ptr = fdt_getprop((void *)fdt, offset, "linux,initrd-end", &len);
        if (ptr) {
            if (len == 4) rg[idx].end = phys_to_virt(fdt32_to_cpu(ptr));
            else rg[idx].end = phys_to_virt(fdt64_to_cpu(ptr));
        }
    ...
    if (offset >= 0){
        ...
        p = (const uint32_t *)align((uintptr_t)((const char *)p + strlen((const char *)p) + 1), 4);
        ...
        while(fdt32_to_cpu(p) != FDT_BEGIN_NODE){
            if(fdt32_to_cpu(p) == FDT_PROP){
                ...
                p += 2; 
                p = (const uint32_t *)align((uintptr_t)((const char *)p + len), 4);
            }
            else{
                p++;
            }
        ...
        while (fdt32_to_cpu(p) == FDT_BEGIN_NODE) {
            ...
            if(prop){ 
                rg[idx].start = phys_to_virt(fdt64_to_cpu(prop));
...
void memory_init(uint64_t fdt){
    ...
    uint64_t mm_start = phys_to_virt(fdt64_to_cpu(prop));
    uint64_t mm_size = fdt64_to_cpu(prop+1);
    uart_printf("mm_start: %x\n", mm_start);
    uart_printf("mm_size: %x\n", mm_size);
