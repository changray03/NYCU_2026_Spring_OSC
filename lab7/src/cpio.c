...
void* cpio_get_all_files(void* current_ptr, const char** name_ptr, uint64_t* size_ptr, void** next_ptr) {
    // 如果傳入 NULL，代表從頭（cpio_base）開始解析
    char* p = (current_ptr == NULL) ? (char*)cpio_base : (char*)current_ptr;
    if (!p) return NULL;

    struct cpio_newc_header* hdr = (struct cpio_newc_header*)p;
    
    // 檢查 Magic Number，如果不對就結束
    if (strncmp(hdr->c_magic, "070701", 6) != 0) return NULL;
    ...
    // 檢查是否到達 CPIO 結尾的 TRAILER!!! 標籤
    if (strcmp(filename, "TRAILER!!!") == 0) return NULL;
    ...
    *size_ptr = filesize;
    ...
    // 計算當前檔案的實體資料（肉體）起始位址
    unsigned int data_offset = align(sizeof(struct cpio_newc_header) + namesize, 4);
    void* actual_data = (void*)(p + data_offset);

    // 計算下一個檔案控制塊的起始位址，供下一輪迭代使用
    *next_ptr = (void*)(p + data_offset + align(filesize, 4));
