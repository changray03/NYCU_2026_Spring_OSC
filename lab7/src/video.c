...
uint16_t __bswaphi2(uint16_t x) {
    return (uint16_t)((x >> 8) | (x << 8));
}

// 32-bit byte swap (對應 __builtin_bswap32，即編譯器報錯的 __bswapsi2)
...
// 64-bit byte swap (對應 __builtin_bswap64，即編譯器報錯的 __bswapdi2)
...
#define FW_CFG_BASE   0xffffffc010100000UL
...
static void fw_cfg_dma_transfer(void* address,
    ...
    struct FWCfgDmaAccess access = {
        ...
        .address = bswap64(virt_to_phys((unsigned long)address)),
    };
    //*FW_CFG_DMA = bswap64((uint64_t)&access);
    *FW_CFG_DMA = bswap64(virt_to_phys((unsigned long)&access));
...
void video_init() {
    struct RAMFBCfg cfg = {
        .addr = bswap64(virt_to_phys(FB_BASE)),
